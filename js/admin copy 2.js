...
async function loadCategoriesAndProducts(hkdId) {
    ...
    try {
        ...
        // Populate category filter và select trong modal
...
async function saveCategory() {
    ...
    try {
        ...
        Utils.showToast('Đã lưu danh mục thành công', 'success');
        ...
        Utils.showToast('Lỗi lưu danh mục', 'error');
    }
}
...
async function saveProduct() {
    ...
    try {
        // Kiểm tra trùng MSP (chỉ khi thêm mới)
        if (!isEdit) {
            const existingProducts = await getProductsByHKD(hkdId);
            const duplicate = existingProducts.find(p => p.msp === productData.msp);
            if (duplicate) {
        ...
        // Thêm vào sync queue - QUAN TRỌNG: phải có categoryId
        await addToSyncQueue({
            type: 'products',
            data: productData
        });
        ...
        Utils.showToast('Lỗi lưu hàng hóa', 'error');
    }
}
...
async function confirmDelete() {
    ...
    try {
        if (type === 'category') {
            // Lấy thông tin danh mục trước khi xóa để có categoryId
            ...
            await addToSyncQueue({
                ...
                data: { 
                    id: id,
                    hkdId: hkdId,
                    categoryId: id // categoryId chính là id của danh mục
                }
            });
            ...
            for (const product of categoryProducts) {
                ...
                await addToSyncQueue({
                    ...
                    data: { 
                        id: product.id,
                        hkdId: hkdId,
                        categoryId: id // Quan trọng: cần categoryId để xóa đúng đường dẫn
                    }
                });
            }
            
        } else if (type === 'product') {
            // Lấy thông tin sản phẩm trước khi xóa để có categoryId
            ...
            await addToSyncQueue({
                ...
                data: { 
                    id: id,
                    hkdId: hkdId,
                    categoryId: product.categoryId // QUAN TRỌNG: cần categoryId
                }
            });
        }
        ...
        Utils.showToast(`Đã xóa ${type === 'category' ? 'danh mục' : 'hàng hóa'} thành công`, 'success');
