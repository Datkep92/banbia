...
// THÊM HÀM MỚI VÀO sync-manager.js
async function syncEssentialDataForAdmin() {
    console.log('🔄 Admin: Đồng bộ dữ liệu quan trọng...');
    
    try {
        // CHỈ SYNC HKD VÀ INVOICES
        await syncStoreFromFirebase('hkds');
        await syncStoreFromFirebase('invoices');
        
        console.log('✅ Admin: Đã đồng bộ dữ liệu quan trọng');
...
/**
 * Thiết lập listener cho hóa đơn của HKD mới
 */
...
async function listenForRealtimeUpdates() {
    console.log('🎧 Bắt đầu lắng nghe realtime updates (HKD + Invoices)...');
    ...
    try {
        ...
        // ==================== 1. REALTIME HKD ====================
        console.log('👂 Lắng nghe HKD realtime...');
        ...
        hkdsRef.on('child_added', async (snapshot) => {
            ...
            console.log(`🆕 HKD mới realtime: ${hkdData.info.name || hkdId}`);
            ...
            if (!existing) {
                ...
                // UI / notify
                if (typeof handleNewHKDRealtime === 'function') {
                    handleNewHKDRealtime(newHKD);
                }

                // 👉 GẮN LISTENER HÓA ĐƠN CHO HKD MỚI
                setupInvoiceListenerForHKD(newHKD);
            }
        });

        // HKD CẬP NHẬT
        hkdsRef.on('child_changed', async (snapshot) => {
            const hkdId = snapshot.key;
            const hkdData = snapshot.val();

            if (!hkdData || !hkdData.info) return;

            console.log(`✏️ HKD cập nhật realtime: ${hkdId}`);
            ...
            const updatedHKD = {
                ...
                password: hkdData.info.password || '',
                role: 'hkd',
                createdAt: hkdData.info.createdAt,
                lastUpdated: hkdData.info.lastUpdated || new Date().toISOString(),
                _synced: true
            };
        ...
        // ==================== 2. REALTIME INVOICE (LOGIC CŨ – GIỮ NGUYÊN) ====================
        const allHKDs = await getAllHKDs();
        ...
        console.log('✅ Đã bật realtime HKD + Invoices');
...
function setupInvoiceListenerForHKD(hkd) {
    ...
    console.log(`👂 Lắng nghe invoices của HKD: ${hkdId}`);
    ...
    invoicesRef
        ...
        .on('child_added', async (snapshot) => {
            ...
            console.log(`📨 Invoice mới từ HKD ${hkdId}: ${invoiceId}`);
            ...
            if (!existing) {
                ...
                if (typeof window.loadInitialData === 'function') {
                    setTimeout(() => window.loadInitialData(), 500);
                }
